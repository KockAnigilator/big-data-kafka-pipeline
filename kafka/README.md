...
Скрипты переписаны под Python `2.6.x` (убраны типы/`f-строки`, `subprocess.run` и т.п.), чтобы они могли запускаться на вашей Cloudera VM.

## 1) Установка зависимостей на VM
Из корня репозитория (примерно `/home/cloudera/...` или ваш путь):
```bash
python2.6 -m pip install --user -r requirements.txt
```

Нужна библиотека `kafka-python`. Команды HDFS выполняются через вызов `hdfs dfs ...` из Python (через `subprocess`).
...
```bash
python2.6 kafka/batch_producer.py \
...
Запуск:
```bash
python2.6 kafka/consumer_hdfs.py \
  --bootstrap-servers localhost:9092 \
  --topic raw-data \
